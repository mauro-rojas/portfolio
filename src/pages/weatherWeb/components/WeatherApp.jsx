...
export default function weatherApp (){
    ...
    let urlWeather = "https://api.openweathermap.org/data/2.5/weather?appid=61278724c50c328e4c73b194c725631f&lang=es";
    
    let cityUrl = "&q="

    let urlForecast = "https://api.openweathermap.org/data/2.5/forecast?appid=61278724c50c328e4c73b194c725631f&lang=es";
    ...
    useEffect(()=>{
        if(city){
            const getData = async () => {
                ...
                try {
                    const [currentWeatherPromise, forecastPromise] = await Promise.allSettled([
                        fetch(urlWeather),
                        fetch(urlForecast)
                    ]);                    
    ...
    return (
        <>              
            < div className= {styles.searchBarContainer}>
                ...
                />  
                <button
                    className={`${lightMode ? styles.lightModeIcon : styles.darkModeIcon}`}
                    onClick={()=>setLightMode(!lightMode)}
                    
                >
                    <img src={`${lightMode ?  darkModeIcon : lightModeIcon}`} alt="icon" draggable="false" />
                </button>   
            </div> 
            {!showWeatherPanel && 
                <motion.div 
            ...
            }
            {
                ...
                <div className={styles.errorContainer}>
                    <motion.img  className= {`${styles.error} ${lightMode ? styles.errorLight : ""}`} src={errorIcon} alt="Algo salio mal..." 
                        animate={{
            ...
            {   
                ...
                <div className={styles.loadingContainer}>
                    <img  className= {styles.loadingAnimation} src={loadingAnimation} alt="loading..." />                        
                </div>
            }
            <a className={styles.linkIcon} href="https://icons8.com/">icons</a> 
